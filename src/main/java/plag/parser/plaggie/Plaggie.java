...
public class Plaggie {
    ...
    private static ArrayList generateDetectionResults(ArrayList submissions)
		...
		if (!(config.blacklistFile == null || config.blacklistFile.equals(""))) {
			try {
				...
				resultQueue.offer("Blacklist file "+config.blacklistFile+" not found, no blacklist used.");
			}
		...
		TokenSimilarityChecker tokenChecker =
				new SimpleTokenSimilarityChecker(config.minimumMatchLength, codeExcluder);

		// No file excluders currently used, therefore null's
		SubmissionSimilarityChecker checker;
		if (config.cacheTokenLists) {
			checker = new CachingSimpleSubmissionSimilarityChecker(tokenChecker,
					codeTokenizer, new HashMap());
		}
		else {
			checker = new SimpleSubmissionSimilarityChecker(tokenChecker,
					codeTokenizer);
		}
		...
		resultQueue.offer("Starting time: "+startTime);
		resultQueue.offer("Similarity value report threshold: "+config.minimumSubmissionSimilarityValue);
		resultQueue.offer("Memory used/free: "+runtime.totalMemory()+" / "+runtime.freeMemory());
		resultQueue.offer("Running "+totalCalculations+" submission comparisons: ");
	...
	*/
		...
		resultQueue.offer("Ending time: "+(new Date()));
		System.out.println();
		Stats.print(System.out);
     ...
     * configuration parameters.
     */
    private static void generateReport(ArrayList detResults
			, ArrayList submissions) throws Exception {
    	ReportGenerator fileRepGen;
		if (config.fullFileDetectionReports) {
			fileRepGen = new SimpleTextReportGenerator(System.out,
					config.printTokenLists, codeTokenizer);
		} else if (config.statFileDetectionReports) {
			fileRepGen = new SimpleTextSubmissionReportGenerator
					.OnlyStatsReportGenerator(System.out, resultQueue);
		} else {
			fileRepGen = null;
		}
		...
		if (config.htmlReport) {
			repGen = new SimpleHtmlSubmissionReportGenerator(new File(config.htmlDir),
					codeTokenizer, config.maximumDetectionResultsToReport);
		} else {
			repGen = new SimpleTextSubmissionReportGenerator(System.out, resultQueue,
					generateFileDetectionReports, fileRepGen);
		}
		...
		System.out.println("************************************************************************");
		resultQueue.offer("************************************************************************");
		resultQueue.offer("   Plaggie report");
		resultQueue.offer("************************************************************************");
		...
		System.out.println("************************************************************************");
		resultQueue.offer("************************************************************************");
		resultQueue.offer("   End of Plaggie report");
		resultQueue.offer("************************************************************************");
	}
    ...
    private static TokenList createTokenList(String filename
			, CodeTokenizer tokenizer) throws Exception {
     ...
     */
    public static Queue<String> compare(String[] args) {
		...
		try {
			...
			resultQueue.offer("Plaggie - Plagiarism Detection tool");
			...
			if (!fConfFile.exists()) {
				...
				resultQueue.offer("Configuration file "+confFile+" not found!");
				System.exit(1);
			}
			try {
				configure(confFile);
				...
				resultQueue.offer(ce.getMessage());
				System.exit(1);
	    	}
			...
			while ((args.length > argc) && (args[argc].startsWith("-"))) {
				if (args[argc].charAt(1) == 's') {
					...
					resultQueue.offer("Command line config: Setting minimum submission similarity value to "+minSubSim);
				...
				if (args[argc].substring(1).equals("nohtml")) {
					...
					resultQueue.offer("Command line config: No html report generated");
